...
//Öncelikle bizden istenilenin ceaser şifreleme olduğunun farkına vardım. Ve ceaser şifreleme fonksiyonunu yazıyoruz. 
void ceaserSifreleme(char metin[],int anahtar) {
	...
	void ceaserDesifreleme(char metin[], int anahtar) {
		
		ceaserSifreleme(metin, 26-anahtar);
		// ceaser şifreleme fonksiyonunun deşifre etmek için kullanılır. 26ya mod alındığı için 26 - anahtar yani anahtarın tersini vermektedir
	}
...
int main() {
		...
		ceaserSifreleme(metin,anahtar);
		
		printf("sifrelenmis metin: %s",metin);
		
		ceaserDesifreleme(metin,anahtar);
